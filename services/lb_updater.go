...
type LBUpdater interface {
	...
	DeleteStream(backendName string) error
}
...
func (u *lbUpdater) DeleteStream(backendName string) error {
	// avoid concurrent updates
	u.lock.Lock()
	defer u.lock.Unlock()
	...
	if err != nil {
		...
		return errors.Wrapf(err, "failed to delete conf file: %s", filename)
	}
